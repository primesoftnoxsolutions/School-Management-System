...
function SchoolBadge() {
  return (
    <div className="flex h-20 w-20 items-center justify-center rounded-full bg-white text-blue-800">
...
function DateSheetPreview({ selectedClass, examTerm, rows, reportingTime, releaseAt, onRowChange }) {
  return (
    <div className="overflow-hidden rounded-xl border-4 border-blue-900 bg-white p-5 text-blue-950 shadow-[0_18px_44px_rgba(37,99,235,0.14)]">
      <div className="grid grid-cols-[7rem_1fr_7rem] items-start">
        <SchoolBadge />
        <div className="text-center">
          <h3 className="text-5xl font-black uppercase leading-none text-blue-950">Date Sheet</h3>
          <div className="mx-auto mt-2 max-w-sm rounded bg-blue-800 px-8 py-1 text-2xl font-black uppercase text-white">
      ...
      <div className="mt-4 grid overflow-hidden rounded-md border border-blue-300 md:grid-cols-3">
        <div className="border-b border-blue-300 px-4 py-3 text-sm font-black uppercase md:border-b-0 md:border-r">
          Examination: {examTerm}
        </div>
        <div className="border-b border-blue-300 px-4 py-3 text-sm font-black uppercase md:border-b-0 md:border-r">
          Exam Time: {rows[0]?.time || "-"}
        </div>
        <div className="px-4 py-3 text-sm font-black uppercase">Reporting Time: {reportingTime}</div>
      </div>

      <table className="mt-3 w-full border-collapse text-center text-sm">
        <thead className="bg-blue-900 text-white">
          <tr>
            <th className="w-24 border border-blue-400 px-3 py-3">Sr. No.</th>
            <th className="border border-blue-400 px-3 py-3">Subject</th>
            <th className="border border-blue-400 px-3 py-3">Day</th>
            <th className="border border-blue-400 px-3 py-3">Date</th>
            <th className="border border-blue-400 px-3 py-3">Time</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row, index) => (
            <tr key={row.subject}>
              <td className="border border-blue-300 px-3 py-3 text-lg font-black text-blue-800">{index + 1}.</td>
              <td className="border border-blue-300 px-3 py-3 font-black uppercase">{row.subject}</td>
              <td className="border border-blue-300 px-3 py-3 font-black text-blue-900">{formatDate(row.date).split(",")[0].toUpperCase()}</td>
              <td className="border border-blue-300 px-3 py-2">
                <input
                  type="date"
                  className="h-10 rounded-lg border border-blue-200 px-2 text-center font-bold text-blue-900 outline-none"
                  value={row.date}
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-[1fr_18rem]">
        <div className="rounded-md border border-blue-300 p-3 text-xs font-bold text-blue-900">
          <span className="rounded bg-blue-800 px-3 py-1 font-black uppercase text-white">Important Instructions:</span>
          ...
          <p className="mt-2">Portal release: {releaseAt ? new Date(releaseAt).toLocaleString() : "Not scheduled"}</p>
        </div>
        <div className="flex flex-col justify-end text-center text-sm font-black uppercase">
          <span className="border-b border-blue-900 pb-6">&nbsp;</span>
          Principal Signature
        </div>
      </div>
    </div>
...
function ResultCardPreview({ selectedClass, student, term, marks, releaseAt, onMarkChange, editable = true }) {
  ...
  return (
    <div className="overflow-hidden rounded-xl border-4 border-blue-900 bg-white p-5 text-blue-950 shadow-[0_18px_44px_rgba(37,99,235,0.14)]">
      <div className="grid grid-cols-[7rem_1fr_7rem] items-start">
        <SchoolBadge />
        <div className="text-center">
          <h3 className="text-5xl font-black uppercase leading-none text-blue-950">Insaf Grammar School</h3>
      ...
      <div className="mt-4 grid gap-x-12 gap-y-2 rounded-md border border-blue-800 p-3 md:grid-cols-2">
        ...
        <InfoLine label="Father Name" value={student?.fatherName || student?.guardianName} />
      ...
      <div className="mx-auto mt-3 max-w-md overflow-hidden rounded-md border border-blue-400 text-sm">
        <div className="bg-blue-900 py-1 text-center font-black uppercase text-white">Result Summary</div>
      ...
      <div className="mt-6 grid grid-cols-2 gap-10 text-center text-xs font-black uppercase text-blue-900">
        ...
        <div><div className="mx-auto mb-2 h-px w-48 bg-blue-900" />Class Teacher Signature</div>
      </div>
  ...
  return (
    <div className="overflow-hidden rounded-xl border-4 border-blue-900 bg-white p-5 text-blue-950 shadow-[0_18px_44px_rgba(37,99,235,0.14)]">
      <div className="grid grid-cols-[5rem_1fr_5rem] items-start gap-3">
        <SchoolBadge />
        <div className="text-center">
          <h3 className="text-3xl font-black uppercase leading-none text-blue-950">Insaf Grammar School</h3>
...
export default function PaperResultCardManagementPage({ dark = false }) {
  ...
  const searchResult = () => {
    const match = savedResults.find((row) => String(row.rollNo).toLowerCase() === resultSearch.toLowerCase());
    if (!match) return;
    const student = students.find((item) => String(item.admissionNo) === String(match.rollNo) || String(item.rollNo || item.rollNumber) === String(match.rollNo));
    if (student) setSelectedStudentId(student._id);
    setResultTerm(match.term);
    setResultMarks(match.marks || {});
    setResultReleaseAt(match.releaseAt || "");
  };
    ...
    <section className="relative space-y-6">
      <style>{`
        @media print {
          ...
          .result-print-root { display: block !important; position: absolute !important; inset: 0 !important; padding: 18px !important; background: white !important; }
        }
      `}</style>
        ...
        <div className="space-y-5">
          {showPaperForm ? (
            ...
            <div className="grid gap-3 xl:grid-cols-2">
              <Field label="Paper Type">
                <select className={`${inputClass} w-full`} value={paperTerm} onChange={(event) => setPaperTerm(event.target.value)}>
                  {TERMS.map((term) => <option key={term}>{term}</option>)}
                </select>
              </Field>
              <Field label="Book / Subject">
                <select className={`${inputClass} w-full`} value={paperBook} onChange={(event) => setPaperBook(event.target.value)}>
                  {SUBJECTS.map((subject) => <option key={subject}>{subject}</option>)}
                </select>
              </Field>
              <div className="grid gap-3 md:grid-cols-3 xl:col-span-2">
                <Field label="Paper Date">
                  <input className={`${inputClass} w-full`} type="date" value={paperDate} onChange={(event) => setPaperDate(event.target.value)} />
                </Field>
          ...
          <div className={`rounded-2xl border p-5 ${panelClass}`}>
            ...
            <div className="mt-4 space-y-3">
              {paperHistory.length ? paperHistory.map((paper) => (
                <div key={paper.id} className="space-y-2">
                  <p className="text-xs font-black uppercase text-slate-500">Saved: {paper.createdAt}</p>
                  <PaperPreview paper={paper} />
                </div>
              )) : <p className="text-sm font-semibold text-slate-500">Saved papers will appear here.</p>}
            </div>
          </div>
        </div>
      ) : null}
          ...
          {showDateSheetForm ? (
            ...
            >
              <div className="w-full max-w-7xl rounded-2xl bg-blue-50 p-4 shadow-2xl">
                <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <h3 className="text-lg font-black text-slate-950">Create Date Sheet</h3>
                    <p className="mt-0.5 text-xs font-semibold text-slate-600">Set term, dates, exam time, and reporting time.</p>
                  </div>
                  <button type="button" onClick={() => setShowDateSheetForm(false)} className="flex h-9 w-9 items-center justify-center rounded-full bg-white text-lg font-black text-blue-800 shadow-sm">x</button>
                </div>

                <div className={`rounded-2xl border p-4 ${panelClass}`}>
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-7">
              <Field label="Exam Term">
                <select className={`${inputClass} w-full`} value={dateSheetTerm} onChange={(event) => setDateSheetTerm(event.target.value)}>
                  {EXAM_TERMS.map((term) => <option key={term}>{term}</option>)}
                </select>
              </Field>
              <Field label="Exam Starting Date">
                <input className={`${inputClass} w-full`} type="date" value={examStartDate} onChange={(event) => setExamStartDate(event.target.value)} />
              </Field>
              <Field label="Gap After Each Paper">
                <input className={`${inputClass} w-full`} type="number" min="0" value={gapDays} onChange={(event) => setGapDays(event.target.value)} placeholder="Gap days" />
              </Field>
              <Field label="Exam Time">
                <input className={`${inputClass} w-full`} value={paperTime} onChange={(event) => setPaperTime(event.target.value)} placeholder="Exam time" />
              </Field>
              <Field label="Reporting Time">
                <input className={`${inputClass} w-full`} value={reportingTime} onChange={(event) => setReportingTime(event.target.value)} placeholder="Reporting time" />
              </Field>
              <Field label="Release Date & Time">
                <input className={`${inputClass} w-full`} type="datetime-local" value={dateSheetReleaseAt} onChange={(event) => setDateSheetReleaseAt(event.target.value)} />
              </Field>
              <button type="button" onClick={saveDateSheet} className="self-end rounded-xl bg-blue-800 px-5 py-3 text-sm font-black uppercase text-white">
                {dateSheetSaved ? "Update" : "Save"}
              </button>
            </div>
          </div>

                <div className="mt-3 max-h-[calc(100vh-13rem)] overflow-y-auto pr-1">
                  <DateSheetPreview
        ...
        <div className="space-y-5">
          ...
          <div className={`rounded-2xl border p-4 ${panelClass}`}>
            <div className="grid gap-3 md:grid-cols-3 xl:grid-cols-6">
              <Field label="Exam Term">
                <select className={`${inputClass} w-full`} value={resultTerm} onChange={(event) => setResultTerm(event.target.value)}>
                  {EXAM_TERMS.map((term) => <option key={term}>{term}</option>)}
                </select>
              </Field>
              <Field label="Roll No / Student">
                <select className={`${inputClass} w-full`} value={selectedStudentId} onChange={(event) => setSelectedStudentId(event.target.value)}>
                  <option value="">Select student</option>
                  {students.map((student) => (
                    <option key={student._id} value={student._id}>
                      {student.rollNo || student.rollNumber || student.admissionNo} - {getStudentName(student)}
                    </option>
                  ))}
                </select>
              </Field>
              <Field label="Release Date & Time">
                <input className={`${inputClass} w-full`} type="datetime-local" value={resultReleaseAt} onChange={(event) => setResultReleaseAt(event.target.value)} />
              </Field>
            ...
            <p className="mt-4 rounded-xl bg-blue-50 px-4 py-3 text-sm font-black text-blue-800">
              Results completed: {savedResultCount} / {students.length}. Remaining: {Math.max(students.length - savedResultCount, 0)}.
            </p>
          </div>
          {selectedStudent ? (
            <ResultCardPreview selectedClass={selectedClass} student={selectedStudent} term={resultTerm} marks={resultMarks} releaseAt={resultReleaseAt} onMarkChange={updateResultMark} />
          ) : (
          ...
          <div className={`rounded-2xl border p-5 ${panelClass}`}>
            ...
            <div className="mt-4 overflow-hidden rounded-xl border border-blue-100">
              {savedResults.length ? (
                <>
                  <div className="hidden border-b border-blue-100 bg-blue-50 px-4 py-2 text-xs font-black uppercase text-blue-900 md:grid md:grid-cols-[7rem_1fr_1fr_auto] md:gap-3">
                    <span>Roll No</span>
                  ...
                  {savedResults.map((result) => (
                    <div key={result.id} className="grid gap-3 border-b border-blue-50 px-4 py-3 text-sm last:border-b-0 md:grid-cols-[7rem_1fr_1fr_auto]">
                      <span className="font-black text-blue-900">#{result.rollNo}</span>
                      <span className="font-bold text-slate-950">{result.studentName}</span>
