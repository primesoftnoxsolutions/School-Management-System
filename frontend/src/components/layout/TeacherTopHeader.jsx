...
const getTeacherSignatureKey = (user) => `teacher-signature:${user?._id || user?.id || user?.fullName || "teacher"}`;
...
export default function TeacherTopHeader({ user, dark = false, onToggleTheme }) {
  ...
  useEffect(() => {
    setSignature(localStorage.getItem(getTeacherSignatureKey(user)) || "");
  }, [user]);

  const saveSignature = (nextSignature) => {
    if (nextSignature) {
      localStorage.setItem(getTeacherSignatureKey(user), nextSignature);
    } else {
      localStorage.removeItem(getTeacherSignatureKey(user));
    }
