...
export default function StudentActivityMonitor({ dark = false, onToggleTheme, refreshKey = 0 }) {
  ...
  const filteredStudents = useMemo(() => {
    ...
    const hasFilter = Boolean(className || section || query);
  ...
  useEffect(() => {
    ...
    const loadAttendanceTotals = async () => {
      setLoadingAttendance(true);
      try {
        const { data } = await api.get("/students/attendance-totals", {
          params: { ids: ids.join(",") },
        });
        if (!cancelled) setAttendanceTotals(data.data || {});
      } catch {
        if (!cancelled) setAttendanceTotals({});
      } finally {
  ...
  const hasFilter = Boolean(className || section || searchQuery.trim());
  const thClass = dark
    ...
    <div>
      ...
      <div className="px-5 py-5">
        ...
        {!hasFilter ? (
          ...
          >
            Select class, section or search by student name to view results.
          </div>
        ) : loading ? (
