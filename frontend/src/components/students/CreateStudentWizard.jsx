...
const GRADE_OPTIONS = ["A+", "A", "B+", "B", "C", "D", "Pass", "Fail", "N/A"];
...
export const initialCreateStudentForm = {
  studentPhotoUrl: "",
  registrationNo: createRegistrationNumber(),
  fullName: "",
  cnicBForm: "",
  address: "",
  phoneNumber: "",
...
export default function CreateStudentWizard({
  ...
  const gradeOptions = useMemo(
    () => [{ value: "", label: "Select result grade" }, ...GRADE_OPTIONS.map((item) => ({ value: item, label: item }))],
    []
  );
  ...
  useEffect(() => {
    ...
    api
      ...
      .then(({ data }) => {
        if (!cancelled) update({ rollNumber: data.data?.rollNumber || "" });
      })
      .catch(() => {
        if (!cancelled) update({ rollNumber: "" });
      })
      .finally(() => {
  ...
  }, [step, form.className, form.section, assignmentEditMode]);
  ...
  const validateStep = (currentStep) => {
    ...
    if (currentStep === 2) {
      ...
      if (!form.guardianPhone.trim()) return "Phone Number is required.";
      return "";
    }
    if (currentStep === 3) {
      const rows = form.previousResults || [];
      for (let i = 0; i < rows.length; i += 1) {
        const row = rows[i];
        const hasAny = row.previousClass || row.resultGrade || row.percentage || row.documentUrl;
        const hasAll = row.previousClass && row.resultGrade && row.percentage;
        if (hasAny && !hasAll) {
          return `Please complete Previous Class, Result Grade and Percentage for the previous class entry, or clear it.`;
        }
      }
      return "";
    }
        ...
        {step === 1 ? (
          <div className="scrollbar-app h-full space-y-4 overflow-y-auto pr-1">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <Field label="Registration Number" dark={dark}>
                <input
              ...
              <Field label="CNIC / B-Form" dark={dark}>
                <input
                  className={inputClass(dark)}
                  value={form.cnicBForm}
                  onChange={(e) => update({ cnicBForm: e.target.value })}
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <Field label="CNIC" dark={dark}>
                <input
                  ...
                  onChange={(e) => update({ fatherCnic: e.target.value })}
                  placeholder="XXXXX-XXXXXXX-X"
                />
              </Field>
              ...
              <div className="sm:col-span-2">
                <Field label="Father Occupation" dark={dark}>
                  <input
                    className={inputClass(dark)}
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                      ...
                      <ScrollableSelect
                        ...
                        value={row.resultGrade}
                        options={gradeOptions}
                        onChange={(value) => updatePreviousRow(row.id, { resultGrade: value })}
                        openUpward
                        menuMaxHeight={320}
                        dark={dark}
                      />
                      <Field label="Percentage" dark={dark}>
                        <input
                          className={inputClass(dark)}
                          value={row.percentage}
                          onChange={(e) => updatePreviousRow(row.id, { percentage: e.target.value })}
                          placeholder="e.g. 85%"
                        />
                      </Field>
                    </div>
            ...
            <div className={assignCardClass}>
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                ...
                <ScrollableSelect
                  ...
                  dark={dark}
                />
                <Field label="Roll Number" dark={dark}>
                  <input
          ...
          <div className="scrollbar-app h-full space-y-4 overflow-y-auto pr-1">
            ...
            <div className={assignCardClass}>
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <Field label="Annual Fee (Rs.)" dark={dark}>
                  <input
                    type="number"
                    min="0"
                    step="1"
